...
async function getOrgIdFromHeadersToken(authHeader: string) {
...
scimRoute.post('/Users', passport.authenticate('bearer'), async (req, res) => {
  const ORG_ID = await getOrgIdFromHeadersToken(req.headers.authorization);
...
scimRoute.get('/Users', passport.authenticate('bearer'), async (req, res) => {
  const ORG_ID = getOrgIdFromHeadersToken(req.headers.authorization);
